...
def test_coverage_status_transitions():
    ...
    assert cov["model_architecture|cnn|rf|one_hot|random"]["status"] == "settled"
    assert cov["feature_representation|rf|rf|kmer|random"]["status"] == "inconclusive"
...
def test_uncovered_excludes_settled():
    ...
    assert all(c.cell_id != "model_architecture|cnn|rf|one_hot|random" for c in unc)
