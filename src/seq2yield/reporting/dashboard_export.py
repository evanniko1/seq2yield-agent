...
def build_html(records: list[dict], claims: list[dict], cost: dict | None = None) -> str:
    ...
    for r in reversed(records):
        ...
        het_cell = f"{het['candidate_wins']}/{het['n_series']} (wr {het['win_rate']})" if het else "—"
        run_rows.append(
...
<table><tr><th>run_id</th><th>intervention</th><th>candidate vs baseline</th>
 <th>feat/sampling/scope</th><th>ΔR²</th><th>data-efficiency</th><th>per-series wins</th>
 <th>verdict</th><th>claim</th></tr>
 {''.join(run_rows)}</table>
