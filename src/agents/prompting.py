...
def generator_prompt(n: int, prior: str = "", targets=None) -> tuple[str, str]:
    ...
    user = (f"Propose exactly {n} DISTINCT, controlled experiments, each comparing one "
            ...
            "scope: 'global' (default; per-series models judged by mean across series), "
            "'per_series' (same, but surface where the winner differs across series), or "
            "'pooled' (train ONE model across all series). Use 'pooled' to ask whether a "
            "single model beats per-series specialized models.\n"
