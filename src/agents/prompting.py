...
def generator_prompt(n: int, prior: str = "", targets=None, open_flags=None,
    ...
    user = (f"Propose exactly {n} DISTINCT, controlled experiments, each comparing one "
            ...
            "  - 'feature_representation': does a richer feature_set (kmer/mechanistic/mixed, or a "
            "frozen foundation-model embedding 'embed:<model>' when one is offered in the coverage "
            "targets) beat one_hot for a model? Use a flat model (rf/mlp) and set comparator_model "
            ...
            "  - 'transfer_generalization': does a finding ESTABLISHED on ONE dataset REPLICATE on "
            "ANOTHER? This is transfer OF CONCLUSIONS (NOT weight transfer — input sizes differ). "
            "Set `dataset` to the TARGET dataset to replicate ON, and model_family/comparator_model "
            "(and any feature/sampling/scaling knob) to the comparison you want to re-test; the "
            "harness resolves a matching settled finding from a DIFFERENT dataset and reports "
            "concordant/discordant/inconclusive.\n"
            "DATASET dimension: set `dataset` to any registered dataset (e.g. ecoli 96nt per-series; "
            "yeast/sample_2019/cuperus_2017 pooled) for a DIRECT question, or use "
            "transfer_generalization with `dataset`=the target to replicate a known finding there.\n"
