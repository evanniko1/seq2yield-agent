...
| **K2a (models)** | Integrate remaining apt models smallest→largest: nt-50m, utr-lm, rna-fm, codonbert, dnabert2, nt-250m, evo (the last three/RNA/Evo need their own backends). Protein LMs intentionally excluded (synonymous signal). | compare all apt pretrained reps | L | no |
